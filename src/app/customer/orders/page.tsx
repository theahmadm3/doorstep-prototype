...
const OrderList = ({ title, orders, onConfirmDelivery, isConfirming, isPastOrder, isLoading, onToggle, orderDetails, loadingDetailsId, noOrdersMessage, children }) => {
    if (isLoading) {
        return (
            <div>
...
export default function CustomerOrdersPage() {
    ...
    const { mutate: confirmDeliveryMutation, isPending: isConfirmingMutation } = useMutation({
    ...
    const pastOrderStatuses: OrderStatus[] = ['Delivered', 'Cancelled', 'Picked Up by Customer', 'Rejected'];
    ...
    const tabContent = {
        ...
        active: (
            <OrderList
                ...
                isConfirming={isConfirmingMutation ? (activeOrders.find(o => o.id === (isConfirmingMutation as any)))?.id : undefined}
                isPastOrder={false}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="sticky top-0 z-40 bg-background border-b rounded-none px-0 w-full grid grid-cols-3">
                    <TabsTrigger value="cart" className="gap-2">
                        ...
                        My Cart <Badge variant={activeTab === 'cart' ? "default" : "secondary"}>{unsubmittedOrders.length}</Badge>
                    </TabsTrigger>
                    <TabsTrigger value="active" className="gap-2">
                        <Truck className="h-4 w-4" />
                        Active <Badge variant={activeTab === 'active' ? "default" : "secondary"}>{activeOrders.length}</Badge>
                    </TabsTrigger>
                    <TabsTrigger value="past" className="gap-2">
                        <History className="h-4 w-4" />
                        Past <Badge variant={activeTab === 'past' ? "default" : "secondary"}>{pastOrders.length}</Badge>
                    </TabsTrigger>
                </TabsList>
