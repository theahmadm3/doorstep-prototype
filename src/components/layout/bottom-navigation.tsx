		...
		<>
			...
			>
				<div className="mx-3 mb-2 rounded-[28px] overflow-hidden">
					...
					{/* Content */}
					<div className="relative flex h-16 items-center justify-around px-2 font-medium bg-white">
						{links.map(({ href, label, icon: Icon }) => {
							...
							return (
								<Link
									...
									className={cn(
										"group relative flex flex-col items-center justify-center px-3 py-1 rounded-2xl transition-all duration-300",
										isActive && "text-primary",
									)}
								>
									{/* Active indicator */}
									{isActive && (
										<div className="absolute inset-0 rounded-2xl bg-primary/10 dark:bg-primary/20 backdrop-blur-xl animate-in fade-in zoom-in-95 duration-300" />
									)}

									{/* Hover / active feedback */}
									<div className="absolute inset-0 rounded-2xl bg-white/10 dark:bg-white/5 opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-all duration-300 backdrop-blur-xl" />

									<Icon
										aria-hidden="true"
										className={cn(
											"relative z-10 mb-1.5 h-6 w-6 transition-transform duration-300 group-hover:scale-110 group-active:scale-95",
