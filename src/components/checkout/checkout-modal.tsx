...
export default function CheckoutModal({
	...
	useEffect(() => {
		if (
			orderType === "delivery" &&
			effectiveRestaurantLat &&
			effectiveRestaurantLng &&
	...
	const handleApplyDiscount = async () => {
		...
		try {
			...
			if (currentSubtotal < result.min_order_value) {
				throw new Error(
					`Discount only applies to orders ₦${result.min_order_value}+`,
				);
			}
			...
			toast({
				...
				description: result.description || `Code ${result.code} applied.`,
			});
		} catch (error) {
			...
			const message =
				error instanceof Error
					? error.message
					: "The discount code is not valid for this order.";
			toast({
				title: "Invalid Code",
				description: message,
				variant: "destructive",
	...
	const triggerPaymentInitialization = useCallback(async () => {
		...
		try {
			...
			const orderPayload: OrderPayload = {
				...
				delivery_address_id:
					orderType === "delivery" ? selectedAddress?.id : undefined,
		...
		const timer = setInterval(async () => {
			...
			if (Date.now() > deadline) {
				...
				toast({
					...
					description: "We're still confirming your payment. Check your orders for updates.",
				});
				onClose();
				navigate("/customer/orders");
				return;
			...
			try {
				const { paid } = await getOrderPaymentStatus(pendingOrderId);
				if (paid) {
					clearInterval(timer);
					setPaymentState("idle");
	...
	}, [paymentState, pendingOrderId, pendingReference, isOpen, handlePaymentSuccess, toast, onClose, navigate]);
	...
	const handlePayment = async () => {
		...
		if (orderType === "delivery" && distance === null) {
			toast({
				...
				description:
					"We couldn't calculate a delivery fee for this location. Please try pickup instead.",
				variant: "destructive",
			});
			return;
		}
	...
	const isPaymentDisabled =
		...
		isFetchingRestaurantCoords ||
		(orderType === "delivery" &&
			(!selectedAddress || distance === null || deliveryFee > 2500));
	...
	const handleDialogOpenChange = (open: boolean) => {
		if (!open) {
			// Reset payment state when modal is closed so it doesn't persist on reopen
			setPaymentState("idle");
			...
			<AlertDialog open={showHighFeeModal} onOpenChange={setShowHighFeeModal}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This branch is far from you. Your delivery fee is ₦
							{deliveryFee.toLocaleString()}. Would you like to order from a
							closer branch instead, or proceed anyway?
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel onClick={handleFindCloserBranch}>
							Find Closer Branch
						</AlertDialogCancel>
						<AlertDialogAction onClick={handleHighFeeProceed}>
							Proceed Anyway
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
				...
				{paymentState === "awaiting_payment" ? (
					...
					<div className="flex flex-col items-center gap-5 px-8 py-10 text-center">
						...
						<div className="space-y-1.5">
							...
							<p className="text-sm text-muted-foreground">
								Enable push notifications so we can alert you the moment your
								order is picked up, on its way, or delivered.
							</p>
						</div>
						<div className="flex flex-col gap-3 w-full">
							<Button
												...
												<div className="flex justify-between items-start gap-2">
													<div>
														...
														{selectedAddress.nearest_landmark && (
															<p className="text-xs text-muted-foreground">
																Landmark: {selectedAddress.nearest_landmark}
															</p>
														)}
													</div>
													<Button
